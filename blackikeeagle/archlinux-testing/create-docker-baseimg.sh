...
cp -a ./mkimage-arch-pacman.conf "$buildfolder/etc/pacman.conf"
...
imageid=$(tar --exclude=etc/pacman.d/gnupg/S.gpg-agent --numeric-owner --xattrs --acls -C "$buildfolder" -c . | docker import - blackikeeagle/archlinux:testing)
docker tag $imageid blackikeeagle/archlinux:$(date +%Y%m%d)-testing
